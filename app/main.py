...
def get_text_messages(message):
    if message.text == "Привет":
        ...
        keyboard.add(key_10)
        key_0 = types.InlineKeyboardButton(text='Справки', callback_data='spravka')
        keyboard.add(key_0)
        
        
        key_11 = types.InlineKeyboardButton(text='Стипендия', callback_data='stipa')
        keyboard.add(key_11)
        key_12 = types.InlineKeyboardButton(text='Повышенная стипендия', callback_data='pgas')
        keyboard.add(key_12)
        key_13 = types.InlineKeyboardButton(text='Расписание', callback_data='raspisanie')
        keyboard.add(key_13) 
        key_14 = types.InlineKeyboardButton(text='Кураторы групп', callback_data='curators')
        keyboard.add(key_14) 
        key_15 = types.InlineKeyboardButton(text='Допуск к физкультуре', callback_data='PE')
        keyboard.add(key_15) 
        key_16 = types.InlineKeyboardButton(text='Не могу быть на учёбе (заболел, медкомиссия)', callback_data='absence')
        keyboard.add(key_16) 
        key_17 = types.InlineKeyboardButton(text='Календарный учебный график', callback_data='academic_calendar')
        keyboard.add(key_17) 
        key_18 = types.InlineKeyboardButton(text='Учебный план (какие будут дисциплины)', callback_data='curriculum')
        keyboard.add(key_18) 
        key_19 = types.InlineKeyboardButton(text='Элективы', callback_data='electives')
        keyboard.add(key_19) 
        key_20 = types.InlineKeyboardButton(text='Дирекция, территориальный отдел', callback_data='administration')
        keyboard.add(key_20) 
        key_21 = types.InlineKeyboardButton(text='Практика', callback_data='internship')
        keyboard.add(key_21) 
        key_22 = types.InlineKeyboardButton(text='Воинский учёт', callback_data='military_registration')
        keyboard.add(key_22)  
...
def callback_query(call):
    ...
    if  call.data == 'raspredelenie':
        ...
        bot.send_message(call.message.chat.id, "НАПОМИНАНИЕ ☑️ \n\nКак найти календарный учебный график (КУГ) на https://www.tyuiu.ru/ 👩‍💻 \n\nСмотри и сохраняй инструкцию😉 \n\nP.S. Как читать КУГ: \n\n1️⃣Найди строку своего курса и направления подготовки  \n\n2️⃣Каждый столбец - шестидневная учебная неделя. Даты в заголовке таблицы стоят с учётом воскресенья, а самих клеток в столбце шесть, потому что воскресенье - выходной по умолчанию \n\n3️⃣Находи в строке своего курса и направления столбец с необходимым обозначением (их расшифровка указана перед таблицей) и сверяй его с датами в шапке таблицы")
        with open ('academic_calendar1.png', 'rb') as photo:
            bot.send_photo(call.message.chat.id, photo)
        with open ('academic_calendar2.png', 'rb') as photo:
            bot.send_photo(call.message.chat.id, photo)
        with open ('academic_calendar3.png', 'rb') as photo:
            bot.send_photo(call.message.chat.id, photo)
        with open ('academic_calendar4.png', 'rb') as photo:
            bot.send_photo(call.message.chat.id, photo)
        with open ('academic_calendar5.png', 'rb') as photo:
            bot.send_photo(call.message.chat.id, photo)
        with open ('academic_calendar6.png', 'rb') as photo:
            bot.send_photo(call.message.chat.id, photo)
